...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  expect(document.querySelector('img[src="/eleven-views-logo.png"]')).not.toBeNull()
  expect(screen.getByText('ELEVEN VIEWS')).toBeTruthy()
  expect(screen.queryByText('Eleven Views Harness')).toBeNull()
